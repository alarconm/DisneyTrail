import { GameEvent } from '../types/game.types';

export const GOOD_EVENTS: GameEvent[] = [
  // Original events
  {
    id: 'tinkerbell-pixiedust',
    title: 'Tinker Bell Appears!',
    description: 'A tiny glow appears beside the wagon. Tinker Bell sprinkles her magical pixie dust over your party! Everything feels more magical.',
    type: 'good',
    effects: [{ type: 'resource', resource: 'pixieDust', amount: 20 }],
    disneyCharacter: 'tinkerbell',
  },
  {
    id: 'minestrone-mouse',
    title: 'Successful Hunt!',
    description: 'Minestrone has caught a mouse! She looks incredibly proud of herself and presents it like a trophy. Marge pretends not to be impressed.',
    type: 'good',
    effects: [{ type: 'resource', resource: 'food', amount: 10 }],
  },
  {
    id: 'mtg-booster',
    title: 'Rare Find!',
    description: 'You find an abandoned Magic: The Gathering booster pack by the trail! Mike would be SO thrilled. Maybe there\'s a Black Lotus in there?',
    type: 'good',
    effects: [{ type: 'resource', resource: 'goldCoins', amount: 30 }],
  },
  {
    id: 'theater-troupe',
    title: 'Theater Troupe!',
    description: 'A traveling theater troupe invites you to perform! Kristin\'s Teaching Artist skills from NW Children\'s Theater shine brightly. Standing ovation!',
    type: 'good',
    effects: [{ type: 'morale', amount: 30 }],
  },
  {
    id: 'mac-warmth',
    title: "Mac's Cozy Night",
    description: "It's a cold night, but Mac's big fluffy body keeps everyone warm. He purrs so loud it sounds like a small engine. Best space heater ever.",
    type: 'good',
    effects: [
      { type: 'health', target: 'all', amount: 5 },
      { type: 'morale', amount: 10 },
    ],
  },
  {
    id: 'wild-cat-treats',
    title: 'Treat Discovery!',
    description: 'Minestrone has discovered a hidden stash of premium cat treats behind a cactus! How did those get there? Marge sighs. Mac drools.',
    type: 'good',
    effects: [{ type: 'resource', resource: 'catTreats', amount: 15 }],
  },
  {
    id: 'dance-morale',
    title: 'Dance Break!',
    description: 'Kristin teaches the cats a jazz routine from her contemporary dance days. Minestrone nails it. Mac trips but looks adorable. Marge judges from the sidelines.',
    type: 'good',
    effects: [{ type: 'morale', amount: 25 }],
  },
  {
    id: 'friendly-travelers',
    title: 'Friendly Travelers',
    description: 'You meet fellow travelers who share their supplies! They heard about NW Children\'s Theater and wanted to support the arts.',
    type: 'good',
    effects: [
      { type: 'resource', resource: 'food', amount: 30 },
      { type: 'resource', resource: 'firstAidKits', amount: 1 },
    ],
  },
  {
    id: 'perfect-weather',
    title: 'Beautiful Day!',
    description: 'The weather is absolutely perfect. Blue skies, gentle breeze, and not too hot. Even Marge seems pleased. Mac found a sunny spot.',
    type: 'good',
    effects: [{ type: 'morale', amount: 15 }],
  },
  {
    id: 'oregon-duck-blessing',
    title: 'Duck Blessing',
    description: 'An Oregon Duck waddles by and gives you an approving nod. Go Ducks! As a fellow U of O alum, Kristin feels energized!',
    type: 'good',
    effects: [{ type: 'morale', amount: 20 }],
  },
  {
    id: 'hip-hop-practice',
    title: 'Hip Hop Session',
    description: 'Kristin finds the perfect spot for some hip-hop practice! Her moves from Clackamas High School days come flooding back. The cats are confused but supportive.',
    type: 'good',
    effects: [{ type: 'morale', amount: 20 }, { type: 'health', target: 'all', amount: 5 }],
  },
  {
    id: 'marge-wisdom',
    title: 'Marge\'s Wisdom',
    description: 'Marge gives everyone a knowing look that somehow solves all your problems. Mom energy is powerful. Party feels organized and ready.',
    type: 'good',
    effects: [{ type: 'morale', amount: 15 }],
  },
  {
    id: 'lucky-penny',
    title: 'Lucky Find!',
    description: 'You find a shiny penny on the trail! "See a penny, pick it up..." The cats think you\'re weird for collecting tiny metal discs.',
    type: 'good',
    effects: [{ type: 'resource', resource: 'goldCoins', amount: 10 }],
  },
  {
    id: 'concordia-memory',
    title: 'Educational Inspiration',
    description: 'Kristin remembers a valuable lesson from her MA at Concordia University-Portland. Applied knowledge makes the journey easier!',
    type: 'good',
    effects: [{ type: 'morale', amount: 15 }],
  },
  {
    id: 'extra-wheel',
    title: 'Roadside Discovery',
    description: 'You find an abandoned wagon wheel in excellent condition! Someone\'s loss is your gain. Mac tries to roll it. It does not go well.',
    type: 'good',
    effects: [{ type: 'resource', resource: 'wagonWheels', amount: 1 }],
  },
  {
    id: 'remy-visit',
    title: 'A Little Chef!',
    description: 'A tiny rat with a chef\'s hat appears and reorganizes your food supplies! Everything is suddenly more appetizing. Anyone can cook!',
    type: 'good',
    effects: [{ type: 'resource', resource: 'food', amount: 20 }],
    disneyCharacter: 'remy',
  },
  {
    id: 'pascal-camouflage',
    title: 'Pascal\'s Help',
    description: 'A small chameleon changes colors to match your wagon! Pascal is helping you blend in. Minestrone is fascinated and tries to catch him.',
    type: 'good',
    effects: [{ type: 'morale', amount: 10 }],
    disneyCharacter: 'pascal',
  },
  {
    id: 'maximus-guard',
    title: 'Royal Guard',
    description: 'Maximus the horse trots alongside your wagon for protection! He keeps giving Mac suspicious looks though.',
    type: 'good',
    effects: [{ type: 'morale', amount: 25 }],
    disneyCharacter: 'maximus',
  },
  // NEW GOOD EVENTS
  {
    id: 'rainbow-sighting',
    title: 'Rainbow After Rain',
    description: 'A beautiful rainbow arcs across the sky after a light rain! The cats seem mesmerized. Marge claims she saw gold at the end. She did not.',
    type: 'good',
    effects: [{ type: 'morale', amount: 20 }],
  },
  {
    id: 'mac-butterfly',
    title: 'Mac\'s New Friend',
    description: 'A butterfly lands on Mac\'s nose! He goes cross-eyed trying to look at it. The whole party dissolves into giggles. Mac is confused but happy.',
    type: 'good',
    effects: [{ type: 'morale', amount: 15 }],
  },
  {
    id: 'marge-grooming',
    title: 'Grooming Session',
    description: 'Marge insists on grooming everyone. Mac complains. Minestrone escapes twice. But afterward, everyone looks fabulous and feels refreshed!',
    type: 'good',
    effects: [{ type: 'health', target: 'all', amount: 10 }, { type: 'morale', amount: 10 }],
  },
  {
    id: 'minestrone-acrobatics',
    title: 'Acrobat Cat!',
    description: 'Minestrone performs an impressive acrobatic routine, jumping between rocks! A passing family applauds and gives you some supplies.',
    type: 'good',
    effects: [{ type: 'resource', resource: 'food', amount: 15 }, { type: 'morale', amount: 20 }],
  },
  {
    id: 'sunset-views',
    title: 'Spectacular Sunset',
    description: 'You stop to watch the most incredible sunset. Oranges, pinks, and purples paint the sky. Even the cats pause their napping to look.',
    type: 'good',
    effects: [{ type: 'morale', amount: 25 }],
  },
  {
    id: 'roadside-berries',
    title: 'Berry Bonanza',
    description: 'Wild berries grow along the trail! You gather as many as you can carry. Minestrone tries one and makes a hilarious face.',
    type: 'good',
    effects: [{ type: 'resource', resource: 'food', amount: 25 }],
  },
  {
    id: 'kind-farmer',
    title: 'Generous Farmer',
    description: 'A kind farmer waves you down and shares fresh vegetables from their garden. "Y\'all look hungry!" The cats are suspicious of vegetables.',
    type: 'good',
    effects: [{ type: 'resource', resource: 'food', amount: 35 }],
  },
  {
    id: 'cool-stream',
    title: 'Refreshing Stream',
    description: 'You find a crystal-clear stream perfect for refilling water and resting. The cats dip their paws in and shake them dramatically.',
    type: 'good',
    effects: [{ type: 'health', target: 'all', amount: 15 }],
  },
  {
    id: 'mac-cuddles',
    title: 'Cuddle Puddle',
    description: 'Mac initiates a group cuddle. Minestrone pretends she doesn\'t want to join but eventually gives in. Marge supervises. Peak coziness achieved.',
    type: 'good',
    effects: [{ type: 'morale', amount: 30 }, { type: 'health', target: 'all', amount: 5 }],
  },
  {
    id: 'clear-road',
    title: 'Easy Traveling',
    description: 'The road ahead is smooth and well-maintained! No potholes, no obstacles. Mac takes this as a sign he should nap more. He would have anyway.',
    type: 'good',
    effects: [{ type: 'morale', amount: 15 }],
  },
  {
    id: 'found-coins',
    title: 'Treasure Trove',
    description: 'You spot something glinting in the dirt - a small pouch of coins! Someone\'s loss is your gain. Mac tries to eat one. Classic Mac.',
    type: 'good',
    effects: [{ type: 'resource', resource: 'goldCoins', amount: 25 }],
  },
  {
    id: 'cat-massage',
    title: 'Kneading Time',
    description: 'Mac kneads Kristin\'s lap while making biscuits. It\'s oddly therapeutic. The stress of travel melts away. Marge approves.',
    type: 'good',
    effects: [{ type: 'morale', amount: 20 }],
  },
  {
    id: 'singing-birds',
    title: 'Morning Chorus',
    description: 'You wake to beautiful birdsong! The cats are hypnotized, chattering at the birds. It\'s the perfect start to the day.',
    type: 'good',
    effects: [{ type: 'morale', amount: 15 }],
  },
  {
    id: 'merchant-discount',
    title: 'Lucky Deal',
    description: 'A traveling merchant offers you supplies at a steep discount! "For the cute cats," they say. Mac poses dramatically.',
    type: 'good',
    effects: [{ type: 'resource', resource: 'food', amount: 20 }, { type: 'resource', resource: 'catTreats', amount: 10 }],
  },
  {
    id: 'minestrone-lizard',
    title: 'The Great Hunt',
    description: 'Minestrone catches a lizard! She parades around with it for an hour before getting bored. The lizard escapes. Minestrone claims victory anyway.',
    type: 'good',
    effects: [{ type: 'morale', amount: 15 }],
  },
  {
    id: 'shady-spot',
    title: 'Perfect Rest Stop',
    description: 'You find an ideal camping spot with shade, soft ground, and a beautiful view. Even Marge admits it\'s acceptable.',
    type: 'good',
    effects: [{ type: 'health', target: 'all', amount: 20 }],
  },
  {
    id: 'old-friends',
    title: 'Familiar Faces',
    description: 'You bump into former theater colleagues on the road! They share supplies and stories. Kristin\'s heart is full.',
    type: 'good',
    effects: [{ type: 'morale', amount: 35 }, { type: 'resource', resource: 'food', amount: 15 }],
  },
  {
    id: 'roadside-spring',
    title: 'Natural Spring',
    description: 'A natural spring provides fresh, cold water! Everyone drinks their fill. Mac tries to catch the bubbles.',
    type: 'good',
    effects: [{ type: 'health', target: 'all', amount: 10 }],
  },
  {
    id: 'lucky-clover',
    title: 'Four-Leaf Clover',
    description: 'You spot a four-leaf clover! Marge claims she found it first. Good fortune feels imminent.',
    type: 'good',
    effects: [{ type: 'morale', amount: 20 }],
  },
  {
    id: 'free-repairs',
    title: 'Helpful Mechanic',
    description: 'A retired wagon mechanic offers free repairs! "Been fixing these for 40 years," they say. The wagon runs smoother than ever.',
    type: 'good',
    effects: [{ type: 'resource', resource: 'wagonWheels', amount: 1 }],
  },
  {
    id: 'starry-night',
    title: 'Stargazing',
    description: 'The night sky is absolutely stunning! You and the cats watch shooting stars. Mac tries to catch one. He cannot.',
    type: 'good',
    effects: [{ type: 'morale', amount: 25 }],
  },
  {
    id: 'marge-purrs',
    title: 'Marge\'s Rare Purr',
    description: 'Marge, the dignified queen, actually purrs! Everyone freezes in amazement. She stops immediately when she notices you noticed.',
    type: 'good',
    effects: [{ type: 'morale', amount: 30 }],
  },
  {
    id: 'wildflowers',
    title: 'Wildflower Fields',
    description: 'You pass through fields of beautiful wildflowers! Minestrone rolls in them. Mac sneezes. Marge judges. But it\'s beautiful.',
    type: 'good',
    effects: [{ type: 'morale', amount: 15 }],
  },
  {
    id: 'tailwind',
    title: 'Helpful Wind',
    description: 'A strong tailwind pushes the wagon along! You make excellent time. The cats\' ears blow back dramatically.',
    type: 'good',
    effects: [{ type: 'morale', amount: 10 }],
  },
  {
    id: 'abandoned-supplies',
    title: 'Supply Cache',
    description: 'You find an abandoned supply cache! Someone must have left it for travelers in need. How fortunate!',
    type: 'good',
    effects: [{ type: 'resource', resource: 'food', amount: 40 }, { type: 'resource', resource: 'firstAidKits', amount: 1 }],
  },
  {
    id: 'cat-nap-sync',
    title: 'Synchronized Napping',
    description: 'All three cats are napping in the exact same position! You snap a photo. This is peak cat ownership. Internet gold!',
    type: 'good',
    effects: [{ type: 'morale', amount: 25 }],
  },
  {
    id: 'minestrone-chirp',
    title: 'Chirping Concert',
    description: 'Minestrone spots a bird and does that adorable chirping thing cats do. Marge joins in. Mac is too lazy but watches with interest.',
    type: 'good',
    effects: [{ type: 'morale', amount: 15 }],
  },
  {
    id: 'perfect-campsite',
    title: 'Dream Campsite',
    description: 'You find THE perfect campsite. Flat ground, shade, view, and even a convenient rock for sitting. The cats immediately claim the best spots.',
    type: 'good',
    effects: [{ type: 'health', target: 'all', amount: 15 }, { type: 'morale', amount: 20 }],
  },
  {
    id: 'mac-belly-up',
    title: 'Belly Rubs!',
    description: 'Mac rolls over for belly rubs! This is a rare honor. He purrs like a motor boat. Everyone takes a turn. Peak happiness achieved.',
    type: 'good',
    effects: [{ type: 'morale', amount: 30 }],
  },
  {
    id: 'fresh-fish',
    title: 'Fresh Catch',
    description: 'You find a stream teeming with fish! The cats are VERY interested. Minestrone actually catches one. She has never been prouder.',
    type: 'good',
    effects: [{ type: 'resource', resource: 'food', amount: 30 }],
  },
  {
    id: 'local-legend',
    title: 'Local Celebrity',
    description: 'A local recognizes Kristin from NW Children\'s Theater! They gush about a show from years ago. Free supplies for the famous actress!',
    type: 'good',
    effects: [{ type: 'resource', resource: 'food', amount: 25 }, { type: 'morale', amount: 30 }],
  },
  {
    id: 'marge-biscuits',
    title: 'Biscuit Time',
    description: 'Marge is making biscuits on the blanket! This dignified queen rarely shows affection, so this is basically a miracle. Hearts melt.',
    type: 'good',
    effects: [{ type: 'morale', amount: 35 }],
  },
  {
    id: 'shortcut-found',
    title: 'Lucky Shortcut',
    description: 'A friendly local shows you a shortcut! "Take a left at the big oak." You make excellent time. The cats sleep through the excitement.',
    type: 'good',
    effects: [{ type: 'morale', amount: 20 }],
  },
  {
    id: 'cool-breeze',
    title: 'Perfect Breeze',
    description: 'A cool breeze cuts through the heat! Everyone sighs with relief. The cats\' fur ruffles majestically. Very photogenic.',
    type: 'good',
    effects: [{ type: 'health', target: 'all', amount: 10 }],
  },
  {
    id: 'free-meal',
    title: 'Generous Inn',
    description: 'A roadside inn offers a free meal! "For the travelers with the cute cats." Mac poses. He knows he\'s the star.',
    type: 'good',
    effects: [{ type: 'resource', resource: 'food', amount: 35 }, { type: 'health', target: 'all', amount: 10 }],
  },
  {
    id: 'disney-music',
    title: 'Musical Moment',
    description: 'You hear Disney music playing from somewhere! A spontaneous sing-along breaks out. Even the cats seem to bob their heads.',
    type: 'good',
    effects: [{ type: 'morale', amount: 30 }],
  },
  {
    id: 'mac-protector',
    title: 'Guard Cat',
    description: 'Mac scared off a suspicious critter by puffing up to enormous size! He looked very fierce. The critter was a squirrel. Still counts.',
    type: 'good',
    effects: [{ type: 'morale', amount: 15 }],
  },
  {
    id: 'minestrone-gift',
    title: 'Present!',
    description: 'Minestrone brings you a "gift" - a leaf she found particularly interesting. She drops it at your feet and meows proudly.',
    type: 'good',
    effects: [{ type: 'morale', amount: 20 }],
  },
  {
    id: 'wagon-upgrade',
    title: 'Free Upgrade',
    description: 'A retired wagon builder admires your rig and offers free upgrades! "She\'s a beauty - let me tune her up."',
    type: 'good',
    effects: [{ type: 'resource', resource: 'wagonWheels', amount: 2 }],
  },
  {
    id: 'healing-herbs',
    title: 'Medicinal Plants',
    description: 'You find healing herbs growing wild! Kristin\'s knowledge from research comes in handy. Natural remedies for everyone!',
    type: 'good',
    effects: [{ type: 'health', target: 'all', amount: 20 }],
  },
  {
    id: 'pixie-hollow',
    title: 'Pixie Hollow',
    description: 'You stumble upon a hidden pixie hollow! Tiny lights dance around. Pixie dust floats everywhere! The cats try to catch it.',
    type: 'good',
    effects: [{ type: 'resource', resource: 'pixieDust', amount: 25 }],
  },
  {
    id: 'cat-cafe',
    title: 'Cat Café',
    description: 'A roadside cat café welcomes you! The cats make new friends. Mac is the biggest cat there. He\'s very smug about it.',
    type: 'good',
    effects: [{ type: 'morale', amount: 25 }, { type: 'resource', resource: 'catTreats', amount: 15 }],
  },
  {
    id: 'golden-hour',
    title: 'Golden Hour',
    description: 'The light is absolutely perfect! Everything glows golden. You take the most beautiful photos. Instagram would explode.',
    type: 'good',
    effects: [{ type: 'morale', amount: 20 }],
  },
  {
    id: 'marge-approval',
    title: 'Marge Approves',
    description: 'Marge gives you a slow blink! This is cat for "I love you." You feel honored. Chosen. Blessed. Today is a good day.',
    type: 'good',
    effects: [{ type: 'morale', amount: 40 }],
  },
];

export const BAD_EVENTS: GameEvent[] = [
  {
    id: 'minestrone-food-raid',
    title: 'Minestrone Strikes Again!',
    description: 'Minestrone got into the food supplies again! She has zero regrets. Those "guilty" eyes say otherwise. Classic troublemaker behavior.',
    type: 'bad',
    effects: [{ type: 'resource', resource: 'food', amount: -30 }],
  },
  {
    id: 'mac-wheel',
    title: 'Mac vs. Wagon',
    description: 'Mac accidentally sat on a wagon wheel. Mac is fine and unbothered. The wheel... significantly less so. He\'s very sorry (he is not).',
    type: 'bad',
    effects: [{ type: 'resource', resource: 'wagonWheels', amount: -1 }],
  },
  {
    id: 'stitch-chaos',
    title: 'Stitch Chaos!',
    description: 'STITCH APPEARED AND CAUSED ABSOLUTE CHAOS! He grabbed something and ran off cackling. "Blue punch buggy! No punch back!"',
    type: 'bad',
    effects: [{ type: 'resource', resource: 'catTreats', amount: -10 }],
    disneyCharacter: 'stitch',
  },
  {
    id: 'marge-hairball',
    title: 'Hairball Delay',
    description: 'Marge has a hairball. A DRAMATIC hairball. She insists on privacy and dignity during this trying time. Travel is delayed.',
    type: 'bad',
    effects: [{ type: 'time', amount: 1 }],
  },
  {
    id: 'traffic-jam',
    title: 'Modern Problems',
    description: 'You have encountered... a traffic jam on I-40. In a wagon. In fantasy Oregon Trail. Some things transcend time and space.',
    type: 'bad',
    effects: [{ type: 'time', amount: 2 }],
  },
  {
    id: 'pothole',
    title: 'Road Hazard!',
    description: 'The wagon hits a massive pothole! Everything rattles violently. Mac thinks it was an earthquake. Marge is profoundly unamused.',
    type: 'bad',
    effects: [{ type: 'morale', amount: -10 }],
  },
  {
    id: 'food-spoiled',
    title: 'Spoiled Supplies',
    description: 'Some of your food has spoiled in the heat. Should have packed more ice... where\'s Elsa when you need her? Oh right, at Crater Lake.',
    type: 'bad',
    effects: [{ type: 'resource', resource: 'food', amount: -25 }],
  },
  {
    id: 'cat-fight',
    title: 'Sibling Rivalry',
    description: 'Minestrone and Mac got into a tiff over the sunny spot in the wagon. Marge had to break it up with her Mom Energy. Party morale decreased.',
    type: 'bad',
    effects: [{ type: 'morale', amount: -15 }],
  },
  {
    id: 'lost-trail',
    title: 'Wrong Turn',
    description: 'You took a wrong turn. Mac was supposed to be navigating but fell asleep. In his defense, he\'s a cat. Classic Mac.',
    type: 'bad',
    effects: [{ type: 'time', amount: 1 }],
  },
  {
    id: 'wagon-squeak',
    title: 'Annoying Squeak',
    description: 'The wagon has developed an annoying squeak. It\'s driving everyone crazy. Minestrone keeps attacking the source. It does not help.',
    type: 'bad',
    effects: [{ type: 'morale', amount: -5 }],
  },
  {
    id: 'mac-nap-spot',
    title: 'Mac Found a Spot',
    description: 'Mac has claimed the driver\'s seat as his new nap spot. He refuses to move. He\'s too heavy to move. Travel is delayed while you negotiate.',
    type: 'bad',
    effects: [{ type: 'time', amount: 1 }],
  },
  {
    id: 'minestrone-zoomies',
    title: 'THE ZOOMIES',
    description: 'Minestrone has the zoomies. AT 3AM. She\'s running laps around the wagon at impossible speeds. Nobody is sleeping tonight.',
    type: 'bad',
    effects: [{ type: 'health', target: 'all', amount: -5 }],
  },
  {
    id: 'bird-distraction',
    title: 'Bird Watching',
    description: 'A bird flew by and all three cats are now frozen, making that weird clicking noise. Travel has stopped. They will not be moved.',
    type: 'bad',
    effects: [{ type: 'time', amount: 1 }],
  },
  {
    id: 'treat-shortage',
    title: 'Treat Crisis',
    description: 'The cats have noticed the treat supplies are running low. They are staging a protest. Marge is the union leader.',
    type: 'bad',
    effects: [{ type: 'morale', amount: -20 }],
  },
  {
    id: 'mac-ate-map',
    title: 'Map Malfunction',
    description: 'Mac ate part of the map. He thought it was a snack. To be fair, it was next to the snacks. This is technically your fault.',
    type: 'bad',
    effects: [{ type: 'time', amount: 1 }],
  },
  // NEW BAD EVENTS
  {
    id: 'mosquitoes',
    title: 'Bug Attack!',
    description: 'Mosquitoes swarm the camp! Everyone is itchy and grumpy. Minestrone tries to eat them. It helps a little.',
    type: 'bad',
    effects: [{ type: 'health', target: 'all', amount: -5 }, { type: 'morale', amount: -10 }],
  },
  {
    id: 'hot-day',
    title: 'Scorching Heat',
    description: 'It\'s unbearably hot! Mac is a puddle. Marge refuses to move. Minestrone is convinced she\'s melting. Everyone is miserable.',
    type: 'bad',
    effects: [{ type: 'health', target: 'all', amount: -10 }],
  },
  {
    id: 'cold-night',
    title: 'Freezing Night',
    description: 'The temperature drops dramatically overnight! Everyone huddles together. Mac finally earns his keep as a space heater.',
    type: 'bad',
    effects: [{ type: 'health', target: 'all', amount: -5 }],
  },
  {
    id: 'muddy-trail',
    title: 'Mud Everywhere',
    description: 'Rain turned the trail to mud! The wagon gets stuck. Mac\'s paws are filthy. Marge is FURIOUS about her fur.',
    type: 'bad',
    effects: [{ type: 'time', amount: 1 }, { type: 'morale', amount: -10 }],
  },
  {
    id: 'minestrone-escape',
    title: 'Escape Artist',
    description: 'Minestrone escaped from the wagon! You spend two hours finding her. She was three feet away, hiding under a rock.',
    type: 'bad',
    effects: [{ type: 'time', amount: 1 }],
  },
  {
    id: 'mac-stuck',
    title: 'Mac Is Stuck',
    description: 'Mac got stuck in a tight spot. Again. It takes forever to free him. He claims the spot was bigger before.',
    type: 'bad',
    effects: [{ type: 'time', amount: 1 }],
  },
  {
    id: 'loud-noises',
    title: 'Sleepless Night',
    description: 'Strange noises keep everyone awake all night. It was probably just owls. Probably. The cats are on high alert.',
    type: 'bad',
    effects: [{ type: 'health', target: 'all', amount: -5 }],
  },
  {
    id: 'marge-mood',
    title: 'Marge Is Displeased',
    description: 'Marge is in A Mood. No one knows why. Everyone walks on eggshells. The judgmental stares are intense.',
    type: 'bad',
    effects: [{ type: 'morale', amount: -20 }],
  },
  {
    id: 'broken-axle',
    title: 'Axle Trouble',
    description: 'The wagon axle is showing wear! You spend time on makeshift repairs. Mac supervised by napping nearby.',
    type: 'bad',
    effects: [{ type: 'time', amount: 1 }],
  },
  {
    id: 'food-ants',
    title: 'Ant Invasion',
    description: 'Ants got into the food supplies! You salvage what you can. Minestrone eats some ants. She seems pleased.',
    type: 'bad',
    effects: [{ type: 'resource', resource: 'food', amount: -20 }],
  },
  {
    id: 'dust-storm',
    title: 'Dust Storm',
    description: 'A dust storm forces you to stop! Everyone\'s eyes are watering. Mac\'s fur is now beige instead of gray.',
    type: 'bad',
    effects: [{ type: 'time', amount: 1 }, { type: 'health', target: 'all', amount: -5 }],
  },
  {
    id: 'steep-hill',
    title: 'Uphill Battle',
    description: 'An unexpectedly steep hill slows progress. The cats "helpfully" add their weight. They do not help push.',
    type: 'bad',
    effects: [{ type: 'time', amount: 1 }],
  },
  {
    id: 'sunburn',
    title: 'Sunburn Situation',
    description: 'Someone forgot sunscreen! Pink ears and noses everywhere. Mac\'s belly is especially pink.',
    type: 'bad',
    effects: [{ type: 'health', target: 'all', amount: -5 }],
  },
  {
    id: 'tangled-supplies',
    title: 'Supply Chaos',
    description: 'Minestrone played with the supply straps. Everything is tangled. It takes forever to sort out. She is not sorry.',
    type: 'bad',
    effects: [{ type: 'time', amount: 1 }],
  },
  {
    id: 'scary-dog',
    title: 'Dog Encounter',
    description: 'A dog barks at the wagon! All three cats puff up to twice their size. Marge hisses. The dog retreats. Morale is shaken.',
    type: 'bad',
    effects: [{ type: 'morale', amount: -15 }],
  },
  {
    id: 'rainy-day',
    title: 'Endless Rain',
    description: 'It won\'t stop raining! Everything is damp. The cats are MISERABLE. Mac shakes water everywhere repeatedly.',
    type: 'bad',
    effects: [{ type: 'morale', amount: -15 }, { type: 'health', target: 'all', amount: -5 }],
  },
  {
    id: 'headwind',
    title: 'Strong Headwind',
    description: 'The wind is blowing directly against you! Progress is slow and frustrating. The cats\' ears won\'t stay down.',
    type: 'bad',
    effects: [{ type: 'time', amount: 1 }],
  },
  {
    id: 'mac-snoring',
    title: 'Mac\'s Snoring',
    description: 'Mac\'s snoring kept everyone awake. All night. It sounded like a chainsaw. He slept perfectly well, of course.',
    type: 'bad',
    effects: [{ type: 'morale', amount: -10 }],
  },
  {
    id: 'treats-ruined',
    title: 'Treat Tragedy',
    description: 'The cat treats got wet and mushy! The cats are outraged. Marge organizes a formal complaint. With her eyes.',
    type: 'bad',
    effects: [{ type: 'resource', resource: 'catTreats', amount: -10 }],
  },
  {
    id: 'rocky-road',
    title: 'Bumpy Trail',
    description: 'The trail is incredibly rocky! Every bump jostles the wagon. Mac gets bounced right off his nap spot.',
    type: 'bad',
    effects: [{ type: 'morale', amount: -10 }],
  },
  {
    id: 'minestrone-midnight',
    title: 'Midnight Madness',
    description: 'Minestrone decided that 3 AM was the perfect time to yowl at absolutely nothing. Everyone is exhausted.',
    type: 'bad',
    effects: [{ type: 'health', target: 'all', amount: -5 }],
  },
  {
    id: 'wasp-nest',
    title: 'Wasp Warning',
    description: 'You accidentally disturbed a wasp nest! Quick evasive action required. Mac knocked over supplies while fleeing.',
    type: 'bad',
    effects: [{ type: 'resource', resource: 'food', amount: -15 }, { type: 'morale', amount: -10 }],
  },
  {
    id: 'detour-needed',
    title: 'Road Closed',
    description: 'The main trail is blocked! A detour adds time to the journey. Mac blames the government.',
    type: 'bad',
    effects: [{ type: 'time', amount: 2 }],
  },
  {
    id: 'marge-hairball-2',
    title: 'Another Hairball',
    description: 'Marge has ANOTHER hairball. This one is somehow worse. She requires extensive recovery time.',
    type: 'bad',
    effects: [{ type: 'time', amount: 1 }],
  },
  {
    id: 'minestrone-sass',
    title: 'Attitude Problem',
    description: 'Minestrone is being extra sassy today. She knocked something off a surface ON PURPOSE. Eye contact was maintained.',
    type: 'bad',
    effects: [{ type: 'morale', amount: -10 }],
  },
  {
    id: 'mac-demands',
    title: 'Demanding Mac',
    description: 'Mac has decided it\'s feeding time. It is not feeding time. He will not accept this. The yowling is ENDLESS.',
    type: 'bad',
    effects: [{ type: 'morale', amount: -15 }],
  },
  {
    id: 'thorny-path',
    title: 'Thorny Trail',
    description: 'Thorny bushes line the path! Everyone gets scratched. Mac got a thorn in his paw and is being VERY dramatic about it.',
    type: 'bad',
    effects: [{ type: 'health', target: 'all', amount: -10 }],
  },
  {
    id: 'wrong-berries',
    title: 'Berry Mistake',
    description: 'Someone ate the wrong berries. Tummy troubles ensue. The cats are fine - they\'re smarter than that. Somehow.',
    type: 'bad',
    effects: [{ type: 'health', target: 'all', amount: -10 }],
  },
  {
    id: 'territorial-cat',
    title: 'Turf War',
    description: 'A local stray cat challenges Mac for territory! Hissing match ensues. Mac retreats. His pride is wounded.',
    type: 'bad',
    effects: [{ type: 'morale', amount: -15 }],
  },
  {
    id: 'marge-refuses',
    title: 'Marge Won\'t Move',
    description: 'Marge has found a spot and refuses to move. You cannot leave without her. She knows this. She uses this power.',
    type: 'bad',
    effects: [{ type: 'time', amount: 1 }],
  },
  {
    id: 'lost-supplies',
    title: 'Lost Supplies',
    description: 'Some supplies fell out of the wagon! You don\'t notice until later. Classic road trip moment. Less classic: the lost food.',
    type: 'bad',
    effects: [{ type: 'resource', resource: 'food', amount: -20 }],
  },
  {
    id: 'rough-terrain',
    title: 'Rough Road',
    description: 'The terrain is brutal! Rocks, ruts, and roots everywhere. The wagon creaks ominously. The cats cling on for dear life.',
    type: 'bad',
    effects: [{ type: 'morale', amount: -10 }, { type: 'health', target: 'all', amount: -5 }],
  },
  {
    id: 'minestrone-trouble',
    title: 'Minestrone\'s Mischief',
    description: 'Minestrone found something she shouldn\'t have. She won\'t show you what. She looks guilty. Something is definitely missing.',
    type: 'bad',
    effects: [{ type: 'resource', resource: 'catTreats', amount: -5 }],
  },
  {
    id: 'suspicious-water',
    title: 'Bad Water',
    description: 'The water source looked fine but tasted weird. Minor stomach upset for everyone. The cats refused to drink it - smart.',
    type: 'bad',
    effects: [{ type: 'health', target: 'all', amount: -5 }],
  },
  {
    id: 'fog-delay',
    title: 'Dense Fog',
    description: 'Fog rolls in so thick you can barely see! Travel is dangerous. You wait it out. The cats think it\'s interesting.',
    type: 'bad',
    effects: [{ type: 'time', amount: 1 }],
  },
  {
    id: 'mac-got-out',
    title: 'Escape Artist Mac',
    description: 'Mac somehow escaped the wagon! He didn\'t go far - he was under it the whole time. But the panic! The searching!',
    type: 'bad',
    effects: [{ type: 'time', amount: 1 }],
  },
  {
    id: 'skunked',
    title: 'Skunk Encounter',
    description: 'A skunk sprayed near the wagon! Nobody got hit directly but the smell... oh, the smell. The cats are OFFENDED.',
    type: 'bad',
    effects: [{ type: 'morale', amount: -20 }],
  },
  {
    id: 'cat-spat',
    title: 'Cat Argument',
    description: 'Marge and Minestrone are having a disagreement. Nobody knows what about. The tension is palpable. Mac hides.',
    type: 'bad',
    effects: [{ type: 'morale', amount: -10 }],
  },
  {
    id: 'flat-tire',
    title: 'Wheel Wobble',
    description: 'A wagon wheel is loose! It takes time to fix. Mac "helps" by sitting on the tools. Thanks, Mac.',
    type: 'bad',
    effects: [{ type: 'time', amount: 1 }],
  },
  {
    id: 'hail-storm',
    title: 'Hail!',
    description: 'A sudden hailstorm! Small but painful ice pellets ping off the wagon. The cats hide under every available blanket.',
    type: 'bad',
    effects: [{ type: 'time', amount: 1 }, { type: 'morale', amount: -10 }],
  },
  {
    id: 'crow-attack',
    title: 'Crow Chaos',
    description: 'A crow steals a snack right out of your hand! The audacity! The cats are equally outraged. Minestrone swears revenge.',
    type: 'bad',
    effects: [{ type: 'resource', resource: 'food', amount: -10 }],
  },
  {
    id: 'nightmare',
    title: 'Bad Dreams',
    description: 'Everyone had nightmares last night. Marge dreamt of empty food bowls. Mac dreamt of... empty food bowls. Cats are simple.',
    type: 'bad',
    effects: [{ type: 'morale', amount: -15 }],
  },
  {
    id: 'bee-sting',
    title: 'Bee Trouble',
    description: 'A bee stings Mac! His face puffs up a little. He looks ridiculous. He knows it. His dignity is in tatters.',
    type: 'bad',
    effects: [{ type: 'health', target: 'Mac', amount: -10 }],
  },
  {
    id: 'pollen-attack',
    title: 'Allergy Season',
    description: 'So much pollen! Everyone is sneezing. Mac\'s sneezes are comically loud. Marge sneezes daintily. Minestrone fights the air.',
    type: 'bad',
    effects: [{ type: 'health', target: 'all', amount: -5 }],
  },
];

export const SPECIAL_EVENTS: GameEvent[] = [
  {
    id: 'elsa-river',
    title: 'Let It Go!',
    description: 'Elsa appears in a magnificent swirl of snow and ice! "The cold never bothered me anyway," she says, freezing the river solid for easy crossing!',
    type: 'special',
    effects: [{ type: 'resource', resource: 'pixieDust', amount: 10 }],
    disneyCharacter: 'elsa',
  },
  {
    id: 'baymax-healing',
    title: 'Healthcare Companion',
    description: 'Baymax waddles over to your wagon. "Hello. I am Baymax, your personal healthcare companion. On a scale of 1 to 10, how would you rate your pain?"',
    type: 'special',
    effects: [{ type: 'health', target: 'all', amount: 25 }],
    disneyCharacter: 'baymax',
  },
  {
    id: 'rapunzel-healing',
    title: 'Healing Hair',
    description: 'Rapunzel appears with her magical golden hair glowing! She sings a healing song. "Flower gleam and glow, let your power shine..." All ailments fade!',
    type: 'special',
    effects: [{ type: 'health', target: 'all', amount: 30 }],
    disneyCharacter: 'rapunzel',
  },
  {
    id: 'goofy-repair',
    title: 'Goofy to the Rescue!',
    description: 'Goofy appears with a toolbox! "Gawrsh, looks like you need some help!" He fixes your wagon... mostly. "A-hyuck! Good as new!"',
    type: 'special',
    effects: [{ type: 'resource', resource: 'wagonWheels', amount: 1 }],
    disneyCharacter: 'goofy',
  },
  {
    id: 'mike-merchant',
    title: 'Familiar Face!',
    description: 'Wait... is that MIKE?! He\'s set up a roadside stand selling rare collectibles and trading cards. "Hey honey! How\'s the journey? Need any Magic cards?"',
    type: 'special',
    effects: [{ type: 'morale', amount: 50 }, { type: 'resource', resource: 'goldCoins', amount: 50 }],
  },
  {
    id: 'olaf-cheer',
    title: 'Warm Hugs!',
    description: 'Olaf waddles up to the wagon! "Hi! I\'m Olaf and I like warm hugs!" He seems perfectly fine despite the heat. His happiness is contagious!',
    type: 'special',
    effects: [{ type: 'morale', amount: 35 }],
    disneyCharacter: 'olaf',
  },
  {
    id: 'simba-encounter',
    title: 'The Lion King',
    description: 'Simba appears on a nearby rock, dramatically silhouetted against the sun! "Remember who you are." The cats are starstruck. Also slightly nervous.',
    type: 'special',
    effects: [{ type: 'morale', amount: 40 }],
    disneyCharacter: 'simba',
  },
  {
    id: 'fairy-godmother',
    title: 'Bibbidi-Bobbidi-Boo!',
    description: 'The Fairy Godmother appears in a shower of sparkles! "Even miracles take a little time." She waves her wand and... pumpkins? No wait, supplies!',
    type: 'special',
    effects: [
      { type: 'resource', resource: 'food', amount: 40 },
      { type: 'resource', resource: 'pixieDust', amount: 15 },
    ],
    disneyCharacter: 'fairy-godmother',
  },
  {
    id: 'mushu-guidance',
    title: 'Guardian Dragon',
    description: 'Mushu appears! "I LIVE! So tell me what mortal needs my protection?" The tiny dragon may be small but his confidence is enormous.',
    type: 'special',
    effects: [{ type: 'morale', amount: 25 }],
    disneyCharacter: 'mushu',
  },
  {
    id: 'pocahontas-nature',
    title: 'Colors of the Wind',
    description: 'Pocahontas guides you through a beautiful path! "You can paint with all the colors of the wind." The scenery is breathtaking. Even the cats pause to look.',
    type: 'special',
    effects: [{ type: 'morale', amount: 30 }, { type: 'health', target: 'all', amount: 10 }],
    disneyCharacter: 'pocahontas',
  },
  {
    id: 'woody-buzz-advice',
    title: 'To Infinity and Beyond!',
    description: 'Woody and Buzz appear! "You\'ve got a friend in me!" They share road trip wisdom. Buzz insists this is a "space mission."',
    type: 'special',
    effects: [{ type: 'morale', amount: 35 }],
    disneyCharacter: 'woody-buzz',
  },
  {
    id: 'dory-directions',
    title: 'Just Keep Swimming!',
    description: 'Dory pops up! "Hey, I remember you! ...wait, do I?" She tries to give directions but keeps forgetting. It\'s adorable.',
    type: 'special',
    effects: [{ type: 'morale', amount: 20 }],
    disneyCharacter: 'dory',
  },
  {
    id: 'wall-e-help',
    title: 'Directive: Help!',
    description: 'WALL-E rolls up with EVE! He shows off his collection of interesting treasures. EVE scans the cats suspiciously. They find some useful supplies!',
    type: 'special',
    effects: [{ type: 'resource', resource: 'wagonWheels', amount: 1 }, { type: 'morale', amount: 15 }],
    disneyCharacter: 'wall-e',
  },
  {
    id: 'maui-strength',
    title: 'You\'re Welcome!',
    description: 'Maui lands dramatically! "What can I say except you\'re welcome!" He helps move some obstacles and flexes a lot. Mac is impressed.',
    type: 'special',
    effects: [{ type: 'morale', amount: 30 }],
    disneyCharacter: 'maui',
  },
  {
    id: 'mirabel-blessing',
    title: 'Family Magic',
    description: 'Mirabel appears! "The miracle is you!" She brings the warmth of the Madrigal family. Bruno is hiding nearby, but we don\'t talk about that.',
    type: 'special',
    effects: [{ type: 'health', target: 'all', amount: 20 }, { type: 'morale', amount: 25 }],
    disneyCharacter: 'mirabel',
  },
  {
    id: 'bruno-prophecy',
    title: 'We Don\'t Talk About...',
    description: 'Bruno emerges from the shadows! "I see... good fortune ahead! ...I think." His rats wave hello. Minestrone is VERY interested in the rats.',
    type: 'special',
    effects: [{ type: 'morale', amount: 20 }, { type: 'resource', resource: 'pixieDust', amount: 10 }],
    disneyCharacter: 'bruno',
  },
  {
    id: 'sisu-luck',
    title: 'Dragon Luck!',
    description: 'Sisu the water dragon appears! "I\'m going to be real with you - I\'m not like the best dragon." But her awkward sincerity brings good luck!',
    type: 'special',
    effects: [{ type: 'morale', amount: 25 }, { type: 'resource', resource: 'goldCoins', amount: 30 }],
    disneyCharacter: 'sisu',
  },
  {
    id: 'hercules-help',
    title: 'A True Hero!',
    description: 'Hercules appears! "I won\'t let you down!" He helps clear the path ahead. The cats are startled by his muscles.',
    type: 'special',
    effects: [{ type: 'morale', amount: 35 }],
    disneyCharacter: 'hercules',
  },
  {
    id: 'ariel-curiosity',
    title: 'Part of Your World',
    description: 'Ariel appears near water! She\'s fascinated by your "land things." She trades some ocean treasures for a look at the cats.',
    type: 'special',
    effects: [{ type: 'resource', resource: 'goldCoins', amount: 25 }, { type: 'morale', amount: 20 }],
    disneyCharacter: 'ariel',
  },
  {
    id: 'aladdin-jasmine',
    title: 'A Whole New World',
    description: 'Aladdin and Jasmine fly by on their magic carpet! They toss down some supplies and wave. Minestrone tries to catch the carpet.',
    type: 'special',
    effects: [{ type: 'resource', resource: 'food', amount: 25 }, { type: 'resource', resource: 'pixieDust', amount: 5 }],
    disneyCharacter: 'aladdin',
  },
  {
    id: 'mulan-courage',
    title: 'Honor to Us All',
    description: 'Mulan rides by on Khan! "The greatest gift and honor is having you." Mushu pops out: "AND ME!" Their courage inspires the party!',
    type: 'special',
    effects: [{ type: 'morale', amount: 40 }],
    disneyCharacter: 'mulan',
  },
  {
    id: 'merida-brave',
    title: 'Touch the Sky!',
    description: 'Merida gallops past! "If you had the chance to change your fate... would you?" She shares some Scottish shortbread. The cats approve.',
    type: 'special',
    effects: [{ type: 'resource', resource: 'food', amount: 20 }, { type: 'morale', amount: 25 }],
    disneyCharacter: 'merida',
  },
  {
    id: 'mike-love-note',
    title: 'A Letter From Home',
    description: 'You find a love note hidden in the supplies! It\'s from Mike: "To Kristin - you\'re my favorite adventure. Can\'t wait to see you at Disney World. I love you! - Mike"',
    type: 'special',
    effects: [{ type: 'morale', amount: 100 }],
  },
];

export function getRandomEvent(): GameEvent {
  const rand = Math.random();

  // 35% chance good, 35% chance bad, 30% chance special
  if (rand < 0.35) {
    return GOOD_EVENTS[Math.floor(Math.random() * GOOD_EVENTS.length)];
  } else if (rand < 0.7) {
    return BAD_EVENTS[Math.floor(Math.random() * BAD_EVENTS.length)];
  } else {
    return SPECIAL_EVENTS[Math.floor(Math.random() * SPECIAL_EVENTS.length)];
  }
}

export function shouldTriggerEvent(): boolean {
  // 30% chance of event each day
  return Math.random() < 0.3;
}

export const ALL_EVENTS = [...GOOD_EVENTS, ...BAD_EVENTS, ...SPECIAL_EVENTS];
export const TOTAL_EVENTS = ALL_EVENTS.length;
